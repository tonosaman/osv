...
def main():
    ...
    osv = subprocess.Popen('cd ../..; scripts/run.py --qemu-path "%s" --hypervisor qemu --vnc none -m 512 -c1 -i "%s" -u -s -e "--norandom --nomount --noinit /tools/mkfs.so; /tools/cpiod.so --prefix /zfs/zfs/; /zfs.so set compression=off osv" --forward tcp:127.0.0.1:%s-:10000' % ('scripts/run-qemu.sh', image_path,upload_port), shell=True, stdout=subprocess.PIPE)
